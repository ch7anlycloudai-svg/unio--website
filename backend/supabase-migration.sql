...
-- No custom functions required — just plain CREATE TABLE statements.
-- ============================================
...
CREATE POLICY IF NOT EXISTS "Public read access on uploads"
ON storage.objects FOR SELECT
USING (bucket_id = 'uploads');

-- Allow authenticated inserts (service role uploads)
CREATE POLICY IF NOT EXISTS "Service role insert on uploads"
ON storage.objects FOR INSERT
WITH CHECK (bucket_id = 'uploads');

-- Allow authenticated updates
CREATE POLICY IF NOT EXISTS "Service role update on uploads"
ON storage.objects FOR UPDATE
USING (bucket_id = 'uploads');

-- Allow authenticated deletes
CREATE POLICY IF NOT EXISTS "Service role delete on uploads"
...
DO $$ BEGIN
    ...
    END IF;
    -- memberships
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'memberships' AND policyname = 'Service role full access on memberships') THEN
...
-- ============================================
-- DROP the old initialize_schema() function if it exists
-- (no longer needed — the backend checks tables directly)
-- ============================================
