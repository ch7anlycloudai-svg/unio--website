...
async function initializeDatabase() {
    ...
    // Ensure all required tables exist (creates them if missing via direct SQL)
 ...
 * Checks each table by attempting a lightweight query.
 * If a table doesn't exist, creates it via raw SQL through Supabase's REST endpoint.
 */
async function ensureTablesExist() {
    const requiredTables = ['admins', 'news', 'messages', 'page_content', 'memberships', 'hero_slides', 'specialties'];

    // Check which tables already exist by trying a head query on each
    const missingTables = [];
    ...
    throw new Error(
        ...
        'Please run the SQL in backend/supabase-migration.sql in the Supabase SQL Editor to create them. ' +
        'No custom functions are required — the migration file contains only CREATE TABLE statements.'
    );
}
